...
from transformers import AutoTokenizer, AutoModelForCausalLM, Gemma2Config

def main(argv):
    # Parse arguments
    if len(argv) < 2:
        print("Usage: python layer_deletion_gemma.py <layers_to_remove> <model_path>")
        sys.exit(1)
    ...
    # Load model and config (in bfloat16 format)
    ...
    config = Gemma2Config.from_pretrained(model_path)

    # Validate layer indices
    max_layer = config.num_hidden_layers - 1
    for layer in layers_to_remove:
        if layer < 0 or layer > max_layer:
            raise ValueError(f"Invalid layer index {layer}. Model has {config.num_hidden_layers} layers. Valid range is 0-{max_layer}.")
    ...
    # Ensure all weights are in bfloat16 (double check)
    model = model.to(torch.bfloat16)

    # Construct save path, supporting two input path formats
    ...
    # Case 2: Further pruning, path already contains 'layer_removed_model'
        ...
        new_parts = parts
        # Remove last pruning dir and checkpoint
