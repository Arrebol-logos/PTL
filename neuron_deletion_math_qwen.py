...
from transformers import AutoTokenizer, AutoModelForCausalLM, Qwen2Config, Qwen2ForCausalLM
...
def retrive_neuron(filename):
    ...
    (e.g., fwd_up, fwd_down, q, k, v), each part being a dict with layer index (0~31) as key 
    and a set as value.
    """
    activate_neuron = []
...
def main(argv):
    ...
    dataset = re.search(r"(?<=output_neurons/qwen/)[^/]+", math_neuron_address).group(0) 
    ...
    pruned_model = Qwen2ForCausalLM(config)
